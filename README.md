...
| 部署 | Docker Compose (MySQL + SearXNG + 后端) |
...
│   └── src/
│       ├── views/             页面：Chat, AgentManage, ArtifactList, Settings, Login
│       ├── stores/            状态：auth, agent, chat, conversation, workspace, artifact, orchestrator
│       ├── api/               API 请求封装
│       ├── components/        通用组件
│       ├── composables/       组合式函数
│       └── router/            路由 (hash 模式)
├── mateclaw-dev/              Spring Boot 后端
│   ├── mateclaw-server/       主模块 — 启动类 MateClawApplication
│   ├── mateclaw-plugin-api/   插件 SDK
│   ├── mateclaw-plugin-sample/ 示例插件
...
```bash
cd mateclaw-dev
cp .env.example .env           # 编辑 .env 设置数据库密码
docker compose up -d           # http://localhost:18080
```

启动三个容器：MySQL 8.0 · SearXNG 搜索引擎 · MateClaw Server。Flyway 自动建表。
