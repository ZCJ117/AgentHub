package vip.mate.infra.skill.installer;
...
import vip.mate.infra.skill.installer.model.*;
import vip.mate.domain.skill.model.SkillEntity;
import vip.mate.domain.skill.service.SkillFileService;
import vip.mate.domain.skill.service.SkillService;
import vip.mate.domain.skill.workspace.SkillWorkspaceEvent;
...
public class SkillInstaller {
    ...
    public Map<String, Object> installFromBundle(SkillBundle bundle, boolean enable, boolean overwrite,
        ...
        if (skillName == null || skillName.isBlank()) {
            throw new vip.mate.common.exception.MateClawException("err.skill.name_required", "Cannot determine skill name from bundle");
        }

        boolean exists = skillService.listSkills().stream()
                .anyMatch(s -> s.getName().equals(skillName));
        if (exists && !overwrite) {
            throw new vip.mate.common.exception.MateClawException("err.skill.name_exists",
