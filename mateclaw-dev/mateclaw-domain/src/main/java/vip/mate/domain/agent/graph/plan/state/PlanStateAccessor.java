package vip.mate.domain.agent.graph.plan.state;
...
import static vip.mate.domain.agent.graph.plan.state.PlanStateKeys.*;

/**
 * Plan-Execute 类型安全的状态访问器
 * <p>
 * 参照 {@link vip.mate.domain.agent.graph.state.MateClawStateAccessor} 的模式，
...
public final class PlanStateAccessor {
    ...
    /**
     * The {@link vip.mate.domain.agent.context.ChatOrigin} forwarded into graph
     * state by {@code MateClawStateAccessor.OutputBuilder.chatOrigin}.
     * Returns {@link vip.mate.domain.agent.context.ChatOrigin#EMPTY} when nothing
     * was injected (legacy callers / non-channel entry points).
     */
    public vip.mate.domain.agent.context.ChatOrigin chatOrigin() {
        return state.<vip.mate.domain.agent.context.ChatOrigin>value(MateClawStateKeys.CHAT_ORIGIN)
                .orElse(vip.mate.domain.agent.context.ChatOrigin.EMPTY);
    }
