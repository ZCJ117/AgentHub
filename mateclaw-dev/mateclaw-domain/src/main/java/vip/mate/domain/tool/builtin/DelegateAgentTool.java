...
import vip.mate.domain.agent.delegation.SubagentRegistry;
import vip.mate.domain.agent.model.AgentEntity;
import vip.mate.domain.agent.repository.AgentMapper;
import vip.mate.domain.audit.service.AuditEventService;
import vip.mate.infra.channel.web.ChatStreamTracker;
import vip.mate.domain.task.AsyncTaskService;
import vip.mate.domain.task.model.AsyncTaskEntity;
...
public class DelegateAgentTool {
    ...
    @vip.mate.domain.tool.ConcurrencyUnsafe("spawns a child agent session and writes to mate_conversation; serialize to keep session graph deterministic")
    @Tool(description = """
    ...
    @vip.mate.domain.tool.ConcurrencyUnsafe("internally fans out to its own thread pool; outer executor must not double-parallelize")
    @Tool(description = """
    ...
    private String buildInheritedContextPrefix(String parentConversationId) {
        try {
            List<vip.mate.domain.workspace.conversation.model.MessageEntity> messages =
    ...
    /**
     * RFC-03 Lane C2 — format a list of parent {@link vip.mate.domain.workspace.conversation.model.MessageEntity}
    ...
    static String formatInheritedContext(
        ...
        List<vip.mate.domain.workspace.conversation.model.MessageEntity> dialogue = messages.stream()
        ...
        sb.append("--- Parent conversation recent context (")
          ...
          .append(") ---\n");
        for (vip.mate.domain.workspace.conversation.model.MessageEntity m : dialogue) {
    ...
    /**
     * 在群聊上下文中，将委托结果记录到 Orchestrator 任务分派表
     * 通过检查 ConversationEntity.conversationType == "group" 来判断是否为群聊
     *
     * <p>Single / parallel / async delegation paths should call this method
     * after each child agent completes so the Orchestrator assignment row
     * reflects the latest status. Non-group conversations are a no-op
     * (one DB query) so the overhead on the common direct-chat path is
     * negligible.
     *
     * @param parentConversationId 父会话 ID（群聊会话的 conversationId）
     * @param agentName            被委托的 Agent 名称
     * @param success              子 Agent 是否执行成功
     * @param result               子 Agent 返回的结果（摘要用）
     */
    private void recordGroupDelegation(String parentConversationId, String agentName,
                                       boolean success, String result) {
        if (parentConversationId == null || parentConversationId.isBlank()) return;
        try {
            ...
            // Find active assignment for this agent in this conversation
            // Update its status based on success/failure
            log.debug("Group delegation recorded: agent={}, success={}", agentName, success);
        } catch (Exception e) {
            log.warn("Failed to record group delegation: {}", e.getMessage());
        }
    }
