package vip.mate.domain.tool.mcp.service;
...
import vip.mate.domain.tool.mcp.repository.McpServerMapper;
...
public class McpServerService {
     ...
     * "no tools".
     */
    public List<vip.mate.domain.tool.mcp.model.McpToolDescriptor> listToolsByServer(Long id) {
        getById(id); // throws if the server is gone — preserves 404 semantics
        return mcpClientManager.getServerTools(id).stream()
                .map(t -> new vip.mate.domain.tool.mcp.model.McpToolDescriptor(
                        t.name(),
                        t.description(),
                        t.inputSchema()))
                .toList();
