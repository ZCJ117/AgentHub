package vip.mate.domain.wiki.service;
...
import vip.mate.domain.wiki.dto.WikiChunkDraft;
import vip.mate.domain.wiki.event.WikiProcessingEvent;
import vip.mate.domain.wiki.job.WikiKbConfig;
import vip.mate.domain.wiki.job.WikiKbConfigParser;
import vip.mate.domain.wiki.model.WikiKnowledgeBaseEntity;
import vip.mate.domain.wiki.model.WikiPageEntity;
import vip.mate.domain.wiki.model.WikiRawMaterialEntity;
import vip.mate.domain.wiki.sse.WikiProgressBus;
...
public class WikiProcessingService {
    ...
    private vip.mate.domain.llm.service.ModelProviderService modelProviderService;
     ...
     */
    @org.springframework.beans.factory.annotation.Autowired(required = false)
    private vip.mate.domain.llm.failover.ProviderHealthTracker providerHealthTracker;

    @org.springframework.beans.factory.annotation.Autowired(required = false)
    @org.springframework.context.annotation.Lazy
    private vip.mate.domain.wiki.job.WikiProcessingJobService wikiJobService;
     ...
     */
    @org.springframework.beans.factory.annotation.Autowired(required = false)
    private vip.mate.domain.wiki.job.WikiModelRoutingService modelRoutingService;
    ...
    public void processRawMaterial(Long rawId, boolean force) {
        ...
        if (wikiJobService != null) {
            try {
                ...
                jobId = job.getId();
                wikiJobService.transition(jobId, vip.mate.domain.wiki.job.WikiJobStage.ROUTING);
            } catch (Exception e) {
        ...
        try {
            ...
            if (wikiJobService != null && jobId != null) {
                try { wikiJobService.transition(jobId, vip.mate.domain.wiki.job.WikiJobStage.PHASE_A_RUNNING); } catch (Exception ignored) {}
            }
            ...
            if (wikiJobService != null && jobId != null) {
                try {
                    var terminalStage = switch (finalStatus) {
                        case "failed" -> vip.mate.domain.wiki.job.WikiJobStage.FAILED;
                        case "partial" -> vip.mate.domain.wiki.job.WikiJobStage.PARTIAL;
                        case "cancelled" -> vip.mate.domain.wiki.job.WikiJobStage.CANCELLED;
                        default -> vip.mate.domain.wiki.job.WikiJobStage.COMPLETED;
                    };
            ...
            if (nonTerminalSideEffects) {
                eventPublisher.publishEvent(new vip.mate.domain.wiki.event.WikiKbDirtyEvent(this, kb.getId()));
            }
            ...
            if (totalChunks > 0 && !"cancelled".equals(finalStatus)) {
                ...
                WIKI_EXECUTOR.submit(() -> {
                    ...
                    } catch (vip.mate.domain.wiki.job.WikiEmbeddingProviderFailingException ex) {
            ...
            if (wikiJobService != null && jobId != null) {
                try {
                    wikiJobService.transition(jobId, cancelled
                            ? vip.mate.domain.wiki.job.WikiJobStage.CANCELLED
                            : vip.mate.domain.wiki.job.WikiJobStage.FAILED);
                } catch (Exception ignored) {}
            }
    ...
    private int processChunk(WikiKnowledgeBaseEntity kb, WikiRawMaterialEntity raw, String textContent,
        ...
        String llmResponse = callLlmWithResilientRetry(prompt, "chunk of raw=" + raw.getId(),
                kb.getId(), vip.mate.domain.wiki.job.WikiJobStep.CREATE_PAGE);
    ...
    private int processChunkTwoPhase(WikiKnowledgeBaseEntity kb, WikiRawMaterialEntity raw,
        ...
        org.springframework.ai.converter.BeanOutputConverter<vip.mate.domain.wiki.dto.RouteResult> routeConverter =
                useStructured
                        ? new org.springframework.ai.converter.BeanOutputConverter<>(vip.mate.domain.wiki.dto.RouteResult.class)
                        : null;
        ...
        String routeResponse = callLlmWithResilientRetry(routePrompt, "route chunk of raw=" + rawId,
                kbId, vip.mate.domain.wiki.job.WikiJobStep.ROUTE);
        ...
        if (routeConverter != null) {
            try {
                vip.mate.domain.wiki.dto.RouteResult bound = routeConverter.convert(routeResponse);
                if (bound != null) {
                    for (vip.mate.domain.wiki.dto.RoutedPageMeta meta : bound.create()) {
    ...
    private int batchCreatePages(WikiKnowledgeBaseEntity kb, WikiRawMaterialEntity raw,
        ...
        for (int bStart = 0; bStart < createMetas.size(); bStart += batchSize) {
            ...
            String batchResponse = callLlmWithResilientRetry(batchPrompt,
                    ...
                    kbId, vip.mate.domain.wiki.job.WikiJobStep.CREATE_PAGE);
    ...
    private boolean mergeOnePage(WikiKnowledgeBaseEntity kb, WikiRawMaterialEntity raw,
        ...
        String response = callLlmWithResilientRetry(prompt,
                ...
                kbId, vip.mate.domain.wiki.job.WikiJobStep.MERGE_PAGE);
    ...
    private String analyzeDocument(WikiKnowledgeBaseEntity kb, WikiRawMaterialEntity raw, String textContent) {
        ...
        try {
            ...
            String response = callLlmWithResilientRetry(prompt, "analyze doc raw=" + raw.getId(),
                    kb.getId(), vip.mate.domain.wiki.job.WikiJobStep.ROUTE);
     ...
     */
    private ChatModel buildChatModelFor(Long kbId, vip.mate.domain.wiki.job.WikiJobStep step) {
     ...
     */
    private String callLlmWithResilientRetry(Prompt prompt, String ctx, Long kbId, vip.mate.domain.wiki.job.WikiJobStep step) {
        long backoffMs = 1000;
    ...
    private ResolvedChatModel resolveChatModel(Long kbId, vip.mate.domain.wiki.job.WikiJobStep step) {
        if (modelRoutingService != null && kbId != null && step != null) {
            try {
                Long modelId = modelRoutingService.selectModelId(kbId, "heavy_ingest", step);
                ModelConfigEntity model = modelConfigService.getModel(modelId);
     ...
     * vip.mate.domain.llm.failover.ProviderHealthTracker}) so a flapping provider
    ...
    public void repairSinglePage(Long targetPageId, Long modelId) {
        ...
        String response = callLlmWithResilientRetry(prompt, "repair page=" + page.getSlug(),
                kb.getId(), vip.mate.domain.wiki.job.WikiJobStep.MERGE_PAGE);
    ...
    private void processLazyIngest(WikiKnowledgeBaseEntity kb, WikiRawMaterialEntity raw) {
        ...
        try {
            ...
            eventPublisher.publishEvent(new vip.mate.domain.wiki.event.WikiKbDirtyEvent(this, kbId));
