...
import vip.mate.domain.llm.routing.MediaCaptionService;
import vip.mate.domain.llm.routing.MultimodalRouter;
import vip.mate.domain.llm.routing.model.MultimodalRoutingDecision;
import vip.mate.domain.llm.service.ModelCapabilityService;
...
public abstract class BaseAgent {
     ...
     * by {@link vip.mate.domain.llm.service.ModelCapabilityService}). Empty set = unknown model,
     ...
     * {@link vip.mate.domain.agent.AgentService.StreamDelta} the SSE accumulator can
     * persist. Returns an empty Flux when no routing happened this turn so we
     * don't emit zero-value events.
     */
    @SuppressWarnings("unchecked")
    public static reactor.core.publisher.Flux<vip.mate.domain.agent.AgentService.StreamDelta> routingStartupDelta(
