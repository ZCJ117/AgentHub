package vip.mate.domain.agent.graph;
...
import vip.mate.infra.llm.chatmodel.AssistantThinkingRelay;
...
public class NodeStreamingChatHelper {
     ...
     *
     * <p>Stored as {@link vip.mate.domain.llm.failover.FallbackEntry} (providerId +
     * ChatModel) so the chain walker can consult {@link vip.mate.domain.llm.failover.ProviderHealthTracker}
     * — cooldown state is keyed by providerId, not by ChatModel instance.</p>
     */
    private final List<vip.mate.domain.llm.failover.FallbackEntry> fallbackChain;

    /** Optional cache-metrics aggregator; {@code null} in tests or when the bean is absent. */
    private final vip.mate.domain.llm.cache.LlmCacheMetricsAggregator cacheMetrics;

    /** Optional per-provider health tracker; {@code null} in tests or when bean absent. */
    private final vip.mate.domain.llm.failover.ProviderHealthTracker healthTracker;
    ...
    @Deprecated
    public NodeStreamingChatHelper(ChatStreamTracker streamTracker, ChatModel fallbackModel,
                                   vip.mate.domain.llm.cache.LlmCacheMetricsAggregator cacheMetrics) {
     ...
     */
    public NodeStreamingChatHelper(ChatStreamTracker streamTracker,
                                   List<vip.mate.domain.llm.failover.FallbackEntry> fallbackChain,
                                   vip.mate.domain.llm.cache.LlmCacheMetricsAggregator cacheMetrics) {
    ...
    public NodeStreamingChatHelper(ChatStreamTracker streamTracker,
                                   List<vip.mate.domain.llm.failover.FallbackEntry> fallbackChain,
                                   vip.mate.domain.llm.cache.LlmCacheMetricsAggregator cacheMetrics,
                                   vip.mate.domain.llm.failover.ProviderHealthTracker healthTracker) {
     ...
     * the {@link vip.mate.domain.llm.failover.AvailableProviderPool} disabled. Kept
     ...
     * {@link vip.mate.domain.llm.failover.AvailableProviderPool} hookup gates both
    ...
    public NodeStreamingChatHelper(ChatStreamTracker streamTracker,
                                   ...
                                   vip.mate.domain.llm.failover.ProviderHealthTracker healthTracker,
                                   String primaryProviderId,
                                   vip.mate.domain.llm.failover.AvailableProviderPool providerPool) {
    ...
    private static List<vip.mate.domain.llm.failover.FallbackEntry> wrap(ChatModel m) {
        ...
        return m == null ? List.of() : List.of(new vip.mate.domain.llm.failover.FallbackEntry("__legacy__", m));
    }
     ...
     * {@link vip.mate.domain.llm.failover.AvailableProviderPool.RemovalSource} for
     ...
     * {@link vip.mate.domain.llm.failover.ProviderHealthTracker}'s cooldown instead.</p>
     */
    private static vip.mate.domain.llm.failover.AvailableProviderPool.RemovalSource hardRemovalSource(ErrorType type) {
        if (type == null) return null;
        return switch (type) {
            case AUTH_ERROR -> vip.mate.domain.llm.failover.AvailableProviderPool.RemovalSource.AUTH_ERROR;
            case BILLING -> vip.mate.domain.llm.failover.AvailableProviderPool.RemovalSource.BILLING;
            default -> null;
        };
    }
    ...
    private StreamResult streamCallInternal(ChatModel chatModel, Prompt prompt,
        ...
        for (int i = 0; i < fallbackChain.size(); i++) {
            vip.mate.domain.llm.failover.FallbackEntry entry = fallbackChain.get(i);
                ...
                .doOnNext(chatResponse -> {
                    ...
                    if (thinkingDelta != null && !thinkingDelta.isEmpty()) {
                        ...
                        boolean suppressThinking = "off".equalsIgnoreCase(
                                vip.mate.infra.llm.chatmodel.ThinkingLevelHolder.get());
                    ...
                    if (chatResponse.getMetadata() != null && chatResponse.getMetadata().getUsage() != null) {
                        ...
                        var cache = vip.mate.domain.llm.cache.CacheUsageExtractor.extract(usage);
