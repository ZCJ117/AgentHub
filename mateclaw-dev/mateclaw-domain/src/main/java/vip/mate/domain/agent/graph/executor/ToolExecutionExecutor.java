package vip.mate.domain.agent.graph.executor;
...
import vip.mate.domain.tool.builtin.ToolExecutionContext;
...
import vip.mate.domain.agent.graph.state.SourceEvidenceLedger;
import vip.mate.domain.approval.ApprovalWorkflowService;
import vip.mate.infra.channel.web.ChatStreamTracker;
import vip.mate.domain.tool.guard.ToolExecutionGuardHelper;
import vip.mate.domain.tool.guard.ToolGuard;
import vip.mate.domain.tool.guard.ToolGuardResult;
import vip.mate.domain.tool.guard.model.GuardEvaluation;
import vip.mate.domain.tool.guard.model.ToolInvocationContext;
import vip.mate.domain.tool.guard.service.ToolGuardService;
...
public class ToolExecutionExecutor {
     ...
     * {@link vip.mate.domain.tool.ToolConcurrencyRegistry} is wired in (legacy tests,
     * backwards-compatible constructors). New code should annotate the tool
     * method with {@link vip.mate.domain.tool.ConcurrencyUnsafe} instead of editing
     * this list.
     */
    /**
    ...
    private final vip.mate.domain.tool.ToolConcurrencyRegistry concurrencyRegistry;
    /**
     ...
     */
    private vip.mate.domain.skill.runtime.SkillRuntimeService skillRuntimeService;

    public void setSkillRuntimeService(vip.mate.domain.skill.runtime.SkillRuntimeService s) {
    ...
    public ToolExecutionExecutor(AgentToolSet toolSet, ToolGuardService toolGuardService,
                                  ApprovalWorkflowService approvalService, ChatStreamTracker streamTracker,
                                  vip.mate.domain.config.ToolTimeoutProperties toolTimeoutProperties) {
    ...
    public ToolExecutionExecutor(AgentToolSet toolSet, ToolGuardService toolGuardService,
                                  ...
                                  vip.mate.domain.config.ToolTimeoutProperties toolTimeoutProperties,
                                  ToolResultStorage resultStorage,
                                  vip.mate.domain.tool.ToolConcurrencyRegistry concurrencyRegistry) {
    ...
    public ToolExecutionResult execute(List<AssistantMessage.ToolCall> toolCalls,
        ...
        for (int i = 0; i < effectiveCalls.size(); i++) {
            ...
            if (vip.mate.domain.tool.builtin.DelegationContext.currentDepth() > 0) {
                java.util.Set<String> denied = vip.mate.domain.tool.builtin.DelegationContext.childDeniedTools();
