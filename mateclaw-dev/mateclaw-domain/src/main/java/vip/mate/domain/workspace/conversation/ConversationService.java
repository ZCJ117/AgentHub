package vip.mate.domain.workspace.conversation;
...
import vip.mate.domain.approval.ApprovalPlaceholderUtil;
...
import vip.mate.domain.approval.repository.ToolApprovalMapper;
import vip.mate.domain.channel.model.ChannelSessionEntity;
import vip.mate.domain.channel.repository.ChannelSessionMapper;
import vip.mate.domain.task.model.AsyncTaskEntity;
import vip.mate.domain.task.repository.AsyncTaskMapper;
import vip.mate.domain.workspace.conversation.event.ConversationDeletedEvent;
...
import vip.mate.domain.workspace.conversation.vo.ConversationVO;
import vip.mate.domain.workspace.conversation.vo.MessageVO;
...
public class ConversationService {
     ...
     */
    private vip.mate.domain.agent.graph.executor.ToolResultStorage toolResultStorage;

    @org.springframework.beans.factory.annotation.Autowired(required = false)
    public void setToolResultStorage(vip.mate.domain.agent.graph.executor.ToolResultStorage toolResultStorage) {
    ...
    /**
     * List conversations with optional conversationType filter.
     *
     * <p>Excludes archived and deleted conversations by default.
     * <p>获取用户的会话列表（按工作区和会话类型过滤）。
     */
    public List<ConversationVO> listConversations(String username, Long workspaceId, String conversationType) {
        ...
        if (conversationType != null && !conversationType.isBlank()) {
            wrapper.eq(ConversationEntity::getConversationType, conversationType);
        }
    ...
    /**
     * Resolve the internal Long id for a conversationId. Needed by
     * pin / reaction / artifact endpoints that reference rows by their
     * numeric PK.
     *
     * <p>通过 conversationId 获取内部 Long 主键。供 pin / reaction /
     * artifact 等需要数字主键的端点使用。
     */
    public Long getConversationId(String conversationId) {
